...
/// <summary>
/// Tests for the language selection decision logic in <see cref="LanguageSelectionService"/>.
/// Uses the internal <see cref="LanguageSelectionService.CandidateResult"/> type via InternalsVisibleTo.
/// </summary>
public sealed class LanguageSelectionDecisionTests
{
    ...
    public void DecideWinningCandidate_ThrowsWhenNoCandidatesHaveSegments()
    {
        ...
        var options = TranscriptionOptions.LoadFromPath(settingsPath);
        var emptyCandidate = new LanguageSelectionService.CandidateResult(
    ...
    private static LanguageSelectionService.CandidateResult CreateCandidate(
        string code, string displayName, float score)
    {
        return CreateCandidateWithPriority(code, displayName, score, 0);
    }

    private static LanguageSelectionService.CandidateResult CreateCandidateWithPriority(
        string code, string displayName, float score, int priority)
    {
        ...
        return new LanguageSelectionService.CandidateResult(
            language,
            score,
            TimeSpan.FromSeconds(5),
            [acceptedSegment],
